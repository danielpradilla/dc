...
d3.csv("./data/PPR-mini.csv", function(houseSales) {
	...
	houseSales.forEach(function(houseSale) {
		...
		houseSale.monthName = monthNames[houseSale.date.getMonth()];
	...
	monthRowChart
		...
		.group(countPerMonth)
		.colors(d3.scale.category10())
		.ordering(function(d) {return monthNames.indexOf(d.key); })
		.label(function(d) {
			return d.key;
		})
		.title(function(d) {
	...
	dataTable
		...
		.columns([
			...
			function(d) {
				return 'No' == d.not_full_market_price ? 'Si' : 'No';
			},
			function(d) {
				return 'Yes' == d.vat_exclusive ? 'Si': 'No';
			}
		])
		.sortBy(function(d) {
			return d.date;
